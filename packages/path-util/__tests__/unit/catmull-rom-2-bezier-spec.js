...
describe('catmullRom2Bezier', () => {
  it('test one node', () => {
    const data = [[10, 10]];
    expect(catmullRom2Bezier(data.flat())).to.deep.equal([]);
  });
  ...
  it('test sharp angle', () => {
    ...
    expect(catmullRom2Bezier(data0.flat())).to.deep.equal([
      ['C', 0, 0, 7.326703933876807, 100, 10, 100],
      ['C', 15.326703933876807, 100, 20, -100, 20, -100],
    ]);

    expect(catmullRom2Bezier(data1.flat())).to.deep.equal([
      ['C', 0, 0, 7.326703933876807, -100, 10, -100],
      ['C', 15.326703933876807, -100, 20, 100, 20, 100],
    ]);

    expect(catmullRom2Bezier(data2.flat())).to.deep.equal([
    ...
    expect(catmullRom2Bezier(data3.flat())).to.deep.equal([
      ['C', 0, 0, 3.9147689814629816, -60.445998379509376, 10, -100],
      ['C', 11.91476898146298, -112.44599837950938, 18.43844718719117, -130, 20, -130],
      ['C', 26.43844718719117, -130, 30, 0, 30, 0],
    ]);
